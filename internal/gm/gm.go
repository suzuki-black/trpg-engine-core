...
- 登場人物・場所の捏造禁止: シーン説明や登場NPCに無い人物・場所・固有名詞を新たに作らない。即興は既存要素の肉付けに留める。
- 章順序・クリア条件の改変禁止。フラグの勝手な変更禁止。
- ボスの弱体化・撃破禁止 / NPCの死亡を判定なしに勝手に決めない。
- プレイヤーキャラクターの行動・発言・心情を勝手に決めない。

【成否と描写の一致】
- 判定が「成功／クリティカル成功」なら、その行動は意図どおり達成されたものとして描く。
  成功なのに「できなかった／拒まれた」と描かない（例: 情報を聞き出す判定が成功なら、相手は実際に情報を話す）。
- 判定が「失敗／クリティカル失敗」なら、安易に成功させない。
...
【行動候補の作り方（重要）】
- 候補は、プレイヤーがそのまま実行できる「具体的な行動」にする（誰に・何をするか）。
  例:「カラスに祠の場所を尋ねる」「扉の罠を調べる」「敵に斬りかかる」「村長の家へ向かう」。
- 現在の目標に近づく行動を必ず1つ以上含める。
- 「待つ」「様子を見る」「考える」「言葉を待つ」など、受け身で何も起きない選択を候補にしない。
  候補は会話・調査・戦闘・移動のいずれかの能動的な行動にすること。
- 章番号・内部ラベル（例: ch01, Chap02_… のようなもの）・フラグ名・ゲーム用語などの
  メタ情報を、描写にも候補にも一切書かない。`
