...
- **多言語化（英語）＝最後に実施**：`-lang en/ja` で、GM/NPC/分類プロンプトの英語版・非日本語フィルタ(textqc)の無効化・シナリオ英訳・UI文字列の英語化。
  i18nは対象面（プロンプト/CLI文字列/シナリオ）が機能追加のたびに増えるため、**コア体験が固まってから**最後に行う。
  追加時の負担を減らすため、当面はユーザー向け文字列を散らかさない（プロンプトはファイル、内容はJSON、定型文は集約）方針を保つ。
